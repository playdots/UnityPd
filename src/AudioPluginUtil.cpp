...
void FFT::Forward(UnityComplexNumber* data, int numsamples)
{
	...
	{
		const float w0 = kPI / (float)j;
		for(int m = 0; m < j; m++)
		{
			const float t = w0 * m;
			UnityComplexNumber w; w.Set(cosf(t), sinf(t));
...
void RingBuffer::Init(int _length)
{
	length = _length;
...
void RingBuffer::ReadBuffer(float* buffer, int numsamplesTarget, int numsamplesSource, float offset)
{
	...
	)
{
	...
	definition.paramdefs[enumvalue].description = name;
	...
	if(enumvalue >= definition.numparameters)
