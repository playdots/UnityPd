...
public class LoudnessMeterCustomGUI : IAudioEffectPluginGUI
{
	...
	private void DrawCurve(Rect r, float[] curve, float yoffset, float yscale, Color col, float labeloffset)
	{
		...
		{
			GUIHelpers.textStyle10.normal.textColor = new Color(col.r, col.g, col.b, 0.7f);
			GUIHelpers.DrawLine(r.x, peakPos, r.x + r.width, peakPos, GUIHelpers.textStyle10.normal.textColor);
			GUIHelpers.DrawText(r.x + labeloffset, peakPos + 6, 60, string.Format("{0:F1} dB", peakVal));
		}
	}
