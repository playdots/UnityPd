...
{
	static public double kPI = 3.1415926;

	public void DrawSpectrogram(Rect r, bool useLogScale, float[] data, float dB_range, float samplerate, float col_r, float col_g, float col_b, float col_a)
	{
...
public class EqualizerCustomGUI : FilterCurveUI
{
		...
		double magScale)
	{
		...
		AudioCurveRendering.AudioCurveEvaluator d = delegate(float x)
		{
			...
			ComplexD hl = (w * (w * coeffs[ 0] + coeffs[ 1]) + coeffs[ 2]) / (w * (w * coeffs[ 3] + coeffs[ 4]) + 1.0f);
			ComplexD hp = (w * (w * coeffs[ 5] + coeffs[ 6]) + coeffs[ 7]) / (w * (w * coeffs[ 8] + coeffs[ 9]) + 1.0f);
			ComplexD hh = (w * (w * coeffs[10] + coeffs[11]) + coeffs[12]) / (w * (w * coeffs[13] + coeffs[14]) + 1.0f);
			ComplexD h = hh * hp * hl;
		...
		if (Event.current.type == EventType.Repaint)
		{
			...
			if (GUIUtility.hotControl == controlID)
				DrawFilterCurve(
					r,
					coeffs,
					(dragOperation == DragOperation.Low) ? lowGain : 0.0f,
					(dragOperation == DragOperation.Mid) ? midGain : 0.0f,
					(dragOperation == DragOperation.High) ? highGain : 0.0f,
			...
			if(showSpectrum)
			{
				...
				DrawSpectrogram(r, useLogScale, spec, dbRange, samplerate, 0.3f, 1.0f, 0.3f, 0.5f * blend);

				plugin.GetFloatBuffer("OutputSpec", out spec, specLen);
				DrawSpectrogram(r, useLogScale, spec, dbRange, samplerate, 1.0f, 0.3f, 0.3f, 0.5f * blend);
			}
