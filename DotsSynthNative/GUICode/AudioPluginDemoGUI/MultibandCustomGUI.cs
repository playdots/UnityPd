...
public class MultibandCustomGUI : FilterCurveUI
{
		...
		double magScale)
	{
		...
		AudioCurveRendering.AudioCurveEvaluator d = delegate(float x) {
			...
			ComplexD lpf  = ComplexD.Pow ((w * (w * coeffs[ 0] + coeffs[ 1]) + coeffs[ 2]) / (w * (w * coeffs[ 3] + coeffs[ 4]) + 1.0f), filterOrder);
			ComplexD bpf1 = ComplexD.Pow ((w * (w * coeffs[ 5] + coeffs[ 6]) + coeffs[ 7]) / (w * (w * coeffs[ 8] + coeffs[ 9]) + 1.0f), filterOrder);
			ComplexD bpf2 = ComplexD.Pow ((w * (w * coeffs[10] + coeffs[11]) + coeffs[12]) / (w * (w * coeffs[13] + coeffs[14]) + 1.0f), filterOrder);
			ComplexD hpf  = ComplexD.Pow ((w * (w * coeffs[15] + coeffs[16]) + coeffs[17]) / (w * (w * coeffs[18] + coeffs[19]) + 1.0f), filterOrder);
